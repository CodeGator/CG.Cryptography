...
public static partial class CryptographyExtensions
{
    ...
    /// <summary>
    /// This method decrypts the given <see cref="Stream"/> using AES.
    /// </summary>
    /// <param name="cryptography">The cryptography instance to use for the
    /// operation.</param>
    /// <param name="key">The key to use for the operation.</param>
    /// <param name="iv">The IV to use for the operation.</param>
    /// <param name="incomingValue">The incoming stream to use for the 
    /// operation.</param>
    /// <param name="outgoingValue">The outgoing stream to use for the 
    /// operation.</param>
    /// <param name="cancellationToken">A cancellation token.</param>
    /// <returns>A task to perform the operation that returns a decrypted 
    /// bytes.</returns>
    /// <exception cref="ArgumentException">This exception is thrown whenever
    /// one or more arguments are missing, or invalid.</exception>
    /// <exception cref="CryptographicException">This exception is thrown 
    /// whenever the operation fails to complete properly.</exception>
    public static async ValueTask AesDecryptAsync(
        this ICryptography cryptography,
        byte[] key,
        byte[] iv,
        Stream incomingValue,
        Stream outgoingValue,
        CancellationToken cancellationToken = default
        )
    {
        // Validate the parameter(s) before attempting to use them.
        Guard.Instance().ThrowIfNull(cryptography, nameof(cryptography))
            ...
            .ThrowIfFalse(key.LongLength == 32, nameof(key))
            .ThrowIfFalse(iv.LongLength == 16, nameof(iv))
            .ThrowIfNull(incomingValue, nameof(incomingValue))
            .ThrowIfNull(outgoingValue, nameof(outgoingValue));
            ...
            using (var alg = Aes.Create())
            {
                ...
                using (var dec = alg.CreateDecryptor())
                {
                        ...
                        ))
                    {
                        ...
                        // Copy the bytes.
                        await incomingValue.CopyToAsync(
                            cryptoStream
                            ).ConfigureAwait(false);
                    }
                }
